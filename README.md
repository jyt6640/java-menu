...
- [ ] 각 코치의 못 먹는 메뉴 입력 안내 문구를 출력한다.
  - "{코치이름}(이)가 못 먹는 메뉴를 입력해 주세요."
- [ ] 메뉴 추천 결과를 출력한다.
  - "메뉴 추천 결과입니다."
  - [ 구분 | 월요일 | 화요일 | 수요일 | 목요일 | 금요일 ] 형식
  - [ 카테고리 | ... ] 형식
  - [ 코치이름 | ... ] 형식 (각 코치별)
- [ ] 서비스 종료 문구를 출력한다.
  - "추천을 완료했습니다."
...
### 기능
- [ ] 월요일부터 금요일까지 5일간 메뉴를 추천한다.
- [ ] 각 요일마다 카테고리를 무작위로 선택한다.
  - `Randoms.pickNumberInRange(1, 5)` 사용
  - 1: 일식, 2: 한식, 3: 중식, 4: 아시안, 5: 양식
- [ ] 한 주에 같은 카테고리는 최대 2회까지만 선택한다.
  - 2회를 초과하면 다시 카테고리를 선택한다.
- [ ] 선택된 카테고리에서 각 코치의 메뉴를 추천한다.
  - `Randoms.shuffle(menus).get(0)` 사용
- [ ] 각 코치에게 한 주에 중복되지 않는 메뉴를 추천한다.
  - 이미 추천된 메뉴는 제외하고 다시 추천한다.
- [ ] 코치가 못 먹는 메뉴는 추천하지 않는다.
  - 못 먹는 메뉴인 경우 다시 추천한다.

### 예외
- [ ] 코치 이름이 2글자 미만이거나 4글자 초과인 경우
- [ ] 코치가 2명 미만이거나 5명 초과인 경우
- [ ] 못 먹는 메뉴가 2개를 초과하는 경우
